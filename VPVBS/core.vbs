...
class cvpmNudge
	...
	Public Sub SolGameOn(aEnabled)
		Dim obj, ii
		If vpmVPVer >= 61 Then
			For Each obj In mSlingBump : obj.Disabled = Not aEnabled : Next
		ElseIf aEnabled Then
			ii = 0
			For Each obj In mSlingBump
				If TypeName(obj) = "Bumper" Then obj.Force = mForce(ii) Else obj.SlingshotStrength = mForce(ii)
				ii = ii + 1
			Next
		Else
			For Each obj In mSlingBump
